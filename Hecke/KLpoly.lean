import Mathlib.GroupTheory.Coxeter.Rpoly
...
open CoxeterSystem Hecke Classical LaurentPolynomial AddMonoidAlgebra
...
noncomputable def Cs (i : B) : Hecke' cs := qsqinv • (ofHecke cs (T cs (s i) - q • T₁))

lemma iota_fix_Cs (i : B) : iota' cs (Cs cs i) = Cs cs i := by
  sorry

-- lemma Cs_mul (i j : B) : Cs cs i * Cs cs j =
--     q⁻¹ •  (T cs (s i * s j) - q • T cs (s i) - q • T cs (s j) + q^2 • T₁) := by sorry

lemma Cs_mul_comm (i j : B) : Cs cs i * Cs cs j = Cs cs j * Cs cs i := by sorry
...
structure iota_fixed_basis where
  ...
  expr_T (w) : ∃ k : KLpoly cs, C w =
    ∑ x : Set.Icc 1 w, (a cs x w * AddMonoidAlgebra.invert (k.P x w)) • ofHecke cs (T cs x)
